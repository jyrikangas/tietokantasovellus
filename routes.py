...
def quizpage(quiz_id):
    if request.method == "GET":
        ...
        if user_id:
            ...
            return render_template("quizinfo.html", quizname=quizname, average=int(average), best="Your best result on this quiz: " + str(int(best)) + "%", quiz_id=quiz_id, commentlist=allcomments)
        return render_template("quizinfo.html", quizname=quizname, average=int(average), commentlist=allcomments)
